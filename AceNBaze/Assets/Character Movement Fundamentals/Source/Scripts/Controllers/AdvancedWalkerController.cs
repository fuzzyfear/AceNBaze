...
namespace CMF
{
	...
	public class AdvancedWalkerController : Controller
	{
		...
		Vector3 newPosition;
		public bool useMouseInsteadOfWASD = true;
		...
		protected virtual Vector3 CalculateMovementDirectionWithMouse()
		{
			if (characterInput == null)
				return Vector3.zero;
			...
			Ray castPoint = testCamera.ScreenPointToRay(mouse);
			RaycastHit hit;
			if (Input.GetKeyDown(KeyCode.Mouse1) == true && Physics.Raycast(castPoint, out hit, Mathf.Infinity, hitLayers))
			{
				...
				_endPosition = tr.position + Vector3.forward * newPosition.z + Vector3.right * newPosition.x;
		...
		protected Vector3 CalculateMovementVelocity()
		{
			...
			if (useMouseInsteadOfWASD)
			{
				_velocity = CalculateMovementDirectionWithMouse();
			}
